...
feature "Bootstrap Layout" do
  ...
  context 'Grid' do
    ...
    scenario "Page has 3 rows" do
      expect(page).to assert_selector('div.row', :count => 3)
    end

    scenario "Each row has 3 col-lg-4" do
      expect(page).to assert_selector('div.col-lg-4', :count => 9)
    end
  end

  context 'Content' do
    scenario "Each col-lg-4 has <p> and bootstrap button" do
      expect(page).to assert_selector('div.row div.col-lg-4 p', :count => 9)
      expect(page).to assert_selector('div.row div.col-lg-4 .btn', :count => 9)
    end
  end
